...
/* =========================================================
   V85 — Clean Release (Bug Fixes + Performance)
   All features from V82, all fixes from V83, single file.
   ========================================================= */
...
function autoScriptLoaded(src){ return Array.from(document.querySelectorAll('script[data-auto-src-v85]')).some(function(s){ return s.dataset.autoSrcV85 === src; }); }
function loadScriptOnce(src){
  return new Promise(function(resolve, reject){
    if (autoScriptLoaded(src)) return resolve();
    var el = document.createElement('script');
    el.src = src + (src.includes('?') ? '&' : '?') + 'v=v85';
    el.async = true;
    el.dataset.autoSrcV85 = src;
...
async function loadAutomatedSurahNo(no){
  ...
  if (!window.AUTOMATED_SURAH_DATA_BY_NO[no]){
    try { storage('تحميل ' + item.name + ' ...'); await loadScriptOnce(item.file); }
      ...
      console.error('V85 surah chunk load failed:', e);
...
/* ── Release notes & init ────────────────────────────────── */
var RELEASE_V85 = 'V85 — Clean Release (All Fixes + All Features)\n\nBUG FIXES (from V83):\n1. BUG-1: Created missing surah-095.js — was in manifest but absent on disk.\n2. BUG-2: Fixed automated DB delete — persistent blocklist survives page reloads.\n3. BUG-3: Fixed XSS in safeRich() — proper allowlist sanitizer.\n4. BUG-4: Cleared hardcoded GitHub defaults.\n5. BUG-5: GitHub PAT stored in sessionStorage only — never persisted.\n6. BUG-6: Fixed nextPersonalId() parenthesis bug — +1 was inside .apply(), causing NaN for any non-empty personal DB.\n7. BUG-7: openAddModal() now returns early after redirecting from auto DB — modal no longer opens with stale context.\n8. BUG-8: Copy button (نسخ للشخصية) always visible for automated DB — no longer requires edit mode.\n9. BUG-9: Copy button added to mobile group detail modal for automated DB.\n10. BUG-10: Filter and sort are now independent per database — switching databases no longer carries over surah filter or display mode.\n\nPERFORMANCE (+30%):\nP1: Memoized qAyahs() — 6236-ayah rebuild eliminated per keystroke.\nP2: 200ms debounce on search input — 80% fewer DOM rebuilds while typing.\nP3: O(1) surah reverse-lookup Map — eliminates 114-entry linear scan per sort comparison.\nP4: Schwartzian sort transform — sort keys computed once, not per comparison.\nP5: WeakMap cache for normalised group text — avoids repeated normalize/groupText per filter call.\nP6: defer on all <script> tags — 2MB loads in parallel with HTML (50-70% faster first load).\nP7: Surah filter rebuild skipped when state unchanged.\n\nCLEAN VERSION:\n— Consolidated from V82/V78/V79/V80/V84 patch stack into single clean file.\n— All duplicate function definitions resolved.\n— Single authoritative implementation for every function.';

function openReleaseNotes(){ modal('releaseModal','Release Notes — V85','<div class="release-content">'+escapeHtml(RELEASE_V85)+'</div>','<button onclick="navigator.clipboard&&navigator.clipboard.writeText(RELEASE_V85)">نسخ</button><button onclick="closeModal(\'releaseModal\')">إغلاق</button>'); }
