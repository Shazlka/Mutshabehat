...
function openHome(){document.getElementById('home').classList.remove('hidden');document.getElementById('workspace').classList.add('hidden');activeDb=null;setActiveHeaderTab('home');updateHomeCounts()}
...
const V83_VIRTUAL_PREP_LIMIT = 150;
function groupPreviewTextV83(g){
  let verse=(g.verses||[])[0];
  if(!verse) return '';
  let raw=(verse.parts||[]).map(p=>safeText(p.text)).join(' ').replace(/\s+/g,' ').trim();
...
function renderCard(g){
  ...
  let totalVerses=(g.verses||[]).length;
  let totalTags=getTags(g).length;
  let score=g.candidateScore?Number(g.candidateScore):null;
  let preview=groupPreviewTextV83(g);
  let cls=(fav?' is-favorite':'')+(done?' is-completed':'')+(locked?' is-locked':'');
  return `<article class="group v83-group-card${cls}" data-id="${g.id}" data-v83-item="true"><div class="group-head" onclick="toggleGroup(this)"><div class="group-num ${done?'completed':''}" title="اضغط لتغيير حالة الإكمال" onclick="event.stopPropagation();toggleFlag(${g.id},'completed')">${escapeHtml(g.id)}</div><div class="group-title-wrap"><div class="group-meta-row"><span class="v83-score">${score!==null?('%'+escapeHtml(score)):'--'}</span><span class="v83-stat">${totalVerses} آية</span><span class="v83-stat">${totalTags} وسم</span></div><div class="group-title">${highlight(g.title||'بدون عنوان')}</div><div class="group-tags">${getTags(g).map(s=>renderSurahTag(g,s)).join('')}</div>${preview?`<div class="v83-preview">${highlight(preview)}</div>`:''}</div><div class="group-actions">${actions}<button class="icon-btn outline-icon" title="نسخ النص" onclick="event.stopPropagation();copyGroupText(${g.id})">${iconSvg('copy')}</button><button class="icon-btn outline-icon" title="صورة HD" onclick="event.stopPropagation();downloadGroupImage(${g.id})">${iconSvg('camera')}</button></div></div><div class="group-body">${renderGroupBody(g)}</div></article>`;
}
...
function renderActiveGroups(){if(!activeDb)return;saveFiltersV85(activeDb);if(typeof isAutoDbV84==='function'&&isAutoDbV84()&&!selectedSurahFilter){let counter=document.getElementById('counter');if(counter)counter.textContent='اختر سورة لتحميل بياناتها فقط';renderChips(0);renderSurahIndex([]);let groupsBox=document.getElementById('groups');if(groupsBox)groupsBox.innerHTML=(typeof automatedPromptHtmlV84==='function')?automatedPromptHtmlV84():'<div class="hint">اختر سورة لتحميل بياناتها فقط</div>';updateToggleAllButton();buildSurahFilter();return}let list=sortGroups(activeData.filter(passFilters));let counter=document.getElementById('counter');if(counter)counter.textContent='عدد النتائج: '+list.length;renderChips(list.length);renderSurahIndex(list);let groupsBox=document.getElementById('groups');if(groupsBox){groupsBox.setAttribute('data-v83-virtual-ready','true');groupsBox.setAttribute('data-v83-total-items',String(list.length));groupsBox.setAttribute('data-v83-window-size',String(V83_VIRTUAL_PREP_LIMIT));}document.getElementById('groups').innerHTML=list.length?(displayMode==='group-surah'?renderGrouped(list):list.map(renderCard).join('')):'<div class="hint">لا توجد نتائج</div>';updateToggleAllButton();buildSurahFilter()}
...
/* =========================================================
   V90 — Phase 6 Search Engine Upgrade
   - Arabic-tolerant normalization
   - Fuzzy matching (missing/extra letters + near match)
   - Instant ranked results (exact > close > semantic)
   ========================================================= */
const RELEASE_V90_SEARCH=`Release Note — V90 Search Engine Upgrade

Implemented:
- Arabic-tolerant normalization for main app search and Quran add/edit search.
- Fuzzy matching for near words (missing/extra letters supported).
- Instant ranked results with priority: exact > close > semantic.
- Debounced live search to avoid excessive rerendering while typing.
`;

let __v90GroupScoreMap=new Map();
let __v90MainSearchTimer=null;
let __v90QuranSearchTimers={add:null,edit:null};

function normalizeArabicV90(v){
...
function tokensV90(v){
  let t=normalizeArabicV90(v);
  return t?t.split(' ').filter(Boolean):[];
}

function editDistanceV90(a,b){
  a=safeText(a); b=safeText(b);
  if(a===b) return 0;
...
function nearTokenV90(qTok,tTok){
  if(!qTok||!tTok) return false;
  if(tTok.includes(qTok)||qTok.includes(tTok)) return true;
  let maxLen=Math.max(qTok.length,tTok.length);
  if(maxLen<=3) return false;
  let d=editDistanceV90(qTok,tTok);
  if(maxLen<=5) return d<=1;
  return d<=2;
}

function scoreTextMatchV90(query,text){
  let q=normalizeArabicV90(query), t=normalizeArabicV90(text);
  if(!q||!t) return null;
  if(t.includes(q)) return {rank:3,score:300+Math.min(80,q.length)};
  let qTokens=tokensV90(q), tTokens=tokensV90(t);
  if(!qTokens.length||!tTokens.length) return null;
  let exactHits=0,nearHits=0;
  for(let i=0;i<qTokens.length;i++){
    let qt=qTokens[i];
    if(tTokens.includes(qt)){ exactHits++; continue; }
    if(tTokens.some(tt=>nearTokenV90(qt,tt))) nearHits++;
  }
  if(exactHits===qTokens.length) return {rank:2,score:240+exactHits*8};
  let covered=exactHits+nearHits;
  let overlap=covered/qTokens.length;
  if(nearHits>0&&overlap>=0.5) return {rank:2,score:180+Math.round(overlap*40)+nearHits*4};
  if(overlap>=0.34) return {rank:1,score:100+Math.round(overlap*35)};
  return null;
}

function scoreGroupSearchV90(group,query){
  let s=scoreTextMatchV90(query,groupText(group));
  if(!s) return null;
  let boost=0;
  let titleScore=scoreTextMatchV90(query,group.title||'');
  if(titleScore) boost+=18;
  let tags=(getTags(group)||[]).join(' ');
  let tagScore=scoreTextMatchV90(query,tags);
  if(tagScore) boost+=10;
  return s.score+boost;
}

function passFilters(g){
  let q=safeText(document.getElementById('searchInput')?.value||'').trim();
  let st=document.getElementById('statusFilter')?.value||'all';
  if(q){
    let score=scoreGroupSearchV90(g,q);
    if(score==null) return false;
    __v90GroupScoreMap.set(Number(g.id),score);
  }
  ...
  if(advancedFilters.surah&&!getTags(g).some(s=>normalize(s).includes(normalize(advancedFilters.surah))))return false;
  return true;
}
...
function renderActiveGroups(){
  ...
  if(typeof isAutoDbV84==='function'&&isAutoDbV84()&&!selectedSurahFilter){
    let counter=document.getElementById('counter');if(counter)counter.textContent='اختر سورة لتحميل بياناتها فقط';
    renderChips(0);renderSurahIndex([]);
    let groupsBox=document.getElementById('groups');
    if(groupsBox)groupsBox.innerHTML=(typeof automatedPromptHtmlV84==='function')?automatedPromptHtmlV84():'<div class="hint">اختر سورة لتحميل بياناتها فقط</div>';
    updateToggleAllButton();buildSurahFilter();return;
  }

  __v90GroupScoreMap.clear();
  let q=safeText(document.getElementById('searchInput')?.value||'').trim();
  let list=activeData.filter(passFilters);

  if(q){
    let ordered=sortGroups(list.slice());
    let orderMap=new Map(ordered.map((g,i)=>[Number(g.id),i]));
    list=list.slice().sort((a,b)=>{
      let sa=__v90GroupScoreMap.get(Number(a.id))||0;
      let sb=__v90GroupScoreMap.get(Number(b.id))||0;
      if(sb!==sa) return sb-sa;
      return (orderMap.get(Number(a.id))||0)-(orderMap.get(Number(b.id))||0);
    });
  }else{
  ...
  let groupsBox=document.getElementById('groups');
  if(groupsBox){
    groupsBox.setAttribute('data-v83-virtual-ready','true');
    groupsBox.setAttribute('data-v83-total-items',String(list.length));
    groupsBox.setAttribute('data-v83-window-size',String(V83_VIRTUAL_PREP_LIMIT));
    groupsBox.innerHTML=list.length?(displayMode==='group-surah'?renderGrouped(list):list.map(renderCard).join('')):'<div class="hint">لا توجد نتائج</div>';
  }
  updateToggleAllButton();
  buildSurahFilter();
}
...
function highlight(t){
  ...
  if(!q) return s;
  try{
    return s.replace(new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'gi'),m=>`<mark>${m}</mark>`);
  }catch(e){
    return s;
  }
}

function __runQuranSearchNowV90(prefix){
  let input=document.getElementById(prefix+'QSearch'), q=input?.value.trim()||'', box=document.getElementById(prefix+'QResults');
  if(!box)return;
  ['Exact','Close','Total'].forEach(k=>{let el=document.getElementById(prefix+k);if(el)el.textContent='0 :'+k});
  if(!q){box.className='quran-results hint';box.innerHTML='اكتب كلمة لعرض النتائج.';return}

  let exact=[], close=[], semantic=[];
  qAyahs().forEach(a=>{
    let txt=safeText(a.text);
    let m=scoreTextMatchV90(q,txt);
    if(!m) return;
    let hit={ayah:a,score:m.score,rank:m.rank};
    if(m.rank===3) exact.push(hit);
    else if(m.rank===2) close.push(hit);
    else semantic.push(hit);
  });

  exact.sort((x,y)=>y.score-x.score);
  close.sort((x,y)=>y.score-x.score);
  semantic.sort((x,y)=>y.score-x.score);
  let all=exact.concat(close,semantic).slice(0,100);
  let total=exact.length+close.length+semantic.length;

  let e=document.getElementById(prefix+'Exact'), c=document.getElementById(prefix+'Close'), t=document.getElementById(prefix+'Total');
  if(e)e.textContent=exact.length+' :Exact';
  if(c)c.textContent=(close.length+semantic.length)+' :Close';
  if(t)t.textContent=total+' :Total';

  box.className='quran-results';
  box.innerHTML=all.map((row,i)=>{
    let a=row.ayah, target=prefix==='edit'?'edit':'add', exists=quranItemExists(target,a.surah,a.ayahNo), checked=exists?'checked':'';
    return `<div class="quran-result ${exists?'selected':''}">
      <label class="quran-check-line"><input type="checkbox" class="${prefix}QCheck" ${checked} onchange="toggleQuranResult('${prefix}',this)" data-index="${i}" data-surah-no="${a.surahNo||a.surah}" data-ayah="${a.ayahNo}" data-text="${escapeHtml(a.text)}" data-surah="${escapeHtml(a.surah)}"><span>${exists?'مضاف':'تحديد'}</span></label>
      <div class="quran-result-body"><b>${a.surah} — ${a.ayahNo}</b><textarea id="${prefix}QText_${i}" class="quran-result-text" readonly>${escapeHtml(a.text)}</textarea>
      <div class="quran-result-actions"><button onclick="addFullQuranResult('${prefix}',this)">إضافة الآية كاملة</button><button onclick="addSelectedQuranResult('${prefix}',${i},this)">إضافة النص المحدد</button></div></div>
    </div>`;
  }).join('') + `<div class="quran-result-toolbar"><button class="primary" onclick="addCheckedQuran('${prefix}')">إضافة المحدد</button><small>الترتيب: مطابق ثم قريب ثم دلالي.</small></div>`;
}

function runQuranSearch(prefix){
  prefix=(prefix==='edit')?'edit':'add';
  clearTimeout(__v90QuranSearchTimers[prefix]);
  __v90QuranSearchTimers[prefix]=setTimeout(function(){__runQuranSearchNowV90(prefix)},120);
}

(function bindInstantSearchV90(){
  function bind(){
    let si=document.getElementById('searchInput');
    if(!si||si.dataset.v90Bound==='1') return;
    si.dataset.v90Bound='1';
    si.addEventListener('input',function(){
      clearTimeout(__v90MainSearchTimer);
      __v90MainSearchTimer=setTimeout(function(){
        if(activeDb){
          saveFiltersV85(activeDb);
          renderActiveGroups();
        }
  ...
  window.addEventListener('DOMContentLoaded',function(){setTimeout(bind,120)});
  setTimeout(bind,120);
})();

/* =========================================================
   V91 — Phase 8 Performance Optimization
   - Lazy/virtual rendering for large lists
   - Scroll + search render throttling
   - Event/timer cleanup guards
   ========================================================= */
const RELEASE_V91_PERF=`Release Note — V91 Performance Optimization

Implemented:
- Virtualized rendering for large result lists in list mode.
- Debounced/RAF-throttled scroll-driven rerendering.
- Reduced full DOM writes by rendering only visible cards.
- Listener/timer cleanup guards to avoid duplicated bindings.
`;

const V91_VIRTUAL_THRESHOLD=120;
const V91_ESTIMATED_CARD_HEIGHT=220;
const V91_BUFFER_ITEMS=8;
let __v91VirtualState={
  enabled:false,
  fullList:[],
  total:0,
  rowH:V91_ESTIMATED_CARD_HEIGHT,
  container:null,
  scrollEl:null,
  start:-1,
  end:-1,
...
function __v91GetScrollEl(){
  return document.querySelector('.v83-center .v83-scroll') || null;
}

function __v91TeardownVirtual(){
  if(__v91VirtualState.onScroll && __v91VirtualState.scrollEl){
    __v91VirtualState.scrollEl.removeEventListener('scroll', __v91VirtualState.onScroll);
  }
  if(__v91VirtualState.raf) cancelAnimationFrame(__v91VirtualState.raf);
  __v91VirtualState.enabled=false;
  __v91VirtualState.fullList=[];
  __v91VirtualState.total=0;
  __v91VirtualState.start=-1;
  __v91VirtualState.end=-1;
  __v91VirtualState.raf=0;
  __v91VirtualState.onScroll=null;
}

function __v91RenderVirtualWindow(force){
  if(!__v91VirtualState.enabled || !__v91VirtualState.container || !__v91VirtualState.scrollEl) return;
  let list=__v91VirtualState.fullList||[];
  let total=list.length;
  let rowH=__v91VirtualState.rowH||V91_ESTIMATED_CARD_HEIGHT;
  let st=__v91VirtualState.scrollEl.scrollTop||0;
  let vh=__v91VirtualState.scrollEl.clientHeight||800;
  let visible=Math.ceil(vh/rowH)+V91_BUFFER_ITEMS*2;
  let start=Math.max(0,Math.floor(st/rowH)-V91_BUFFER_ITEMS);
  let end=Math.min(total,start+visible);
  if(!force && start===__v91VirtualState.start && end===__v91VirtualState.end) return;
  __v91VirtualState.start=start;
  __v91VirtualState.end=end;
  let topPad=start*rowH;
  let bottomPad=Math.max(0,(total-end)*rowH);
  let slice=list.slice(start,end);
  __v91VirtualState.container.innerHTML=
    `<div class="v91-pad" style="height:${topPad}px"></div>`+
    slice.map(renderCard).join('')+
    `<div class="v91-pad" style="height:${bottomPad}px"></div>`;
}

function __v91SetupVirtualList(list,groupsBox){
  __v91TeardownVirtual();
  let scrollEl=__v91GetScrollEl();
  if(!scrollEl || !groupsBox) return false;
  __v91VirtualState.enabled=true;
  __v91VirtualState.fullList=list;
  __v91VirtualState.total=list.length;
  __v91VirtualState.container=groupsBox;
  __v91VirtualState.scrollEl=scrollEl;
  __v91VirtualState.onScroll=function(){
    if(__v91VirtualState.raf) return;
    __v91VirtualState.raf=requestAnimationFrame(function(){
      __v91VirtualState.raf=0;
      __v91RenderVirtualWindow(false);
    });
  };
  scrollEl.addEventListener('scroll',__v91VirtualState.onScroll,{passive:true});
  __v91RenderVirtualWindow(true);
  return true;
}
...
function renderActiveGroups(){
  ...
  if(groupsBox){
    ...
    groupsBox.setAttribute('data-v83-total-items',String(list.length));
    groupsBox.setAttribute('data-v83-window-size',String(V83_VIRTUAL_PREP_LIMIT));
    let useVirtual=(displayMode!=='group-surah' && list.length>=V91_VIRTUAL_THRESHOLD);
    if(useVirtual){
      let ok=__v91SetupVirtualList(list,groupsBox);
      if(!ok){
        __v91TeardownVirtual();
        groupsBox.innerHTML=list.map(renderCard).join('');
      }
    }else{
      __v91TeardownVirtual();
      groupsBox.innerHTML=list.length?(displayMode==='group-surah'?renderGrouped(list):list.map(renderCard).join('')):'<div class="hint">لا توجد نتائج</div>';
...
window.addEventListener('beforeunload',function(){
  __v91TeardownVirtual();
  clearTimeout(__v90MainSearchTimer);
  clearTimeout(__v90QuranSearchTimers.add);
  clearTimeout(__v90QuranSearchTimers.edit);
});

/* =========================================================
   V92 — Phase 9 UX Polish
   - Theme quick toggle (light/dark)
   - Accessibility UI scale
   - Desktop keyboard shortcuts
   - Focus mode
   ========================================================= */
const V92_UI_SCALE_KEY='mutashabihat_v92_ui_scale';
const V92_FOCUS_KEY='mutashabihat_v92_focus_mode';

function __v92GetScale(){
  let n=Number(localStorage.getItem(V92_UI_SCALE_KEY)||100);
  if(isNaN(n)) n=100;
  return Math.max(90,Math.min(130,n));
}
function __v92ApplyScale(){
  document.documentElement.style.fontSize=__v92GetScale()+'%';
}
function setUiScaleV92(next){
  next=Math.max(90,Math.min(130,Number(next)||100));
  localStorage.setItem(V92_UI_SCALE_KEY,String(next));
  __v92ApplyScale();
  if(typeof toast==='function') toast('حجم الواجهة: '+next+'%','ok');
}

function toggleThemeV92(){
  let s=(typeof getSettings==='function')?getSettings():{};
  let cur=s.theme||document.body.getAttribute('data-theme')||'quran-classic';
  let next=(cur==='bevel-night'||cur==='v83-dark')?'quran-classic':'bevel-night';
  s.theme=next;
  try{localStorage.setItem(SETTINGS_KEY,JSON.stringify(s))}catch(e){}
  if(typeof applySettings==='function') applySettings();
  if(typeof ghRenderV79==='function') ghRenderV79();
  if(typeof toast==='function') toast(next==='quran-classic'?'تم تفعيل النمط الفاتح':'تم تفعيل النمط الليلي','ok');
}

function toggleFocusModeV92(){
  let on=!document.body.classList.contains('v92-focus-mode');
  document.body.classList.toggle('v92-focus-mode',on);
  try{localStorage.setItem(V92_FOCUS_KEY,on?'1':'0')}catch(e){}
}

function __v92PickGroup(step){
  if(!activeDb) return;
  let nodes=[...document.querySelectorAll('#groups .group[data-id]')];
  if(!nodes.length) return;
  let ids=nodes.map(n=>Number(n.getAttribute('data-id'))).filter(n=>!isNaN(n));
  let current=document.querySelector('#groups .group.is-selected-v92');
  let curId=current?Number(current.getAttribute('data-id')):ids[0];
  let i=Math.max(0,ids.indexOf(curId));
  let next=Math.max(0,Math.min(ids.length-1,i+step));
  document.querySelectorAll('#groups .group.is-selected-v92').forEach(x=>x.classList.remove('is-selected-v92'));
  let target=nodes.find(n=>Number(n.getAttribute('data-id'))===ids[next])||nodes[next];
  if(!target) return;
  target.classList.add('is-selected-v92');
  target.scrollIntoView({block:'nearest',behavior:'smooth'});
  let head=target.querySelector('.group-head');
  if(head) head.click();
}
...
(function initV92(){
  ...
  window.addEventListener('DOMContentLoaded',function(){
    __v92ApplyScale();
    if(localStorage.getItem(V92_FOCUS_KEY)==='1') document.body.classList.add('v92-focus-mode');
  });
  document.addEventListener('keydown',function(e){
    if(ignoreTarget(e.target)) return;
    if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){
      e.preventDefault();
      if(typeof openMainSearchUI==='function') openMainSearchUI();
      let si=document.getElementById('searchInput'); if(si) si.focus();
      return;
    }
    if(e.key==='/'&&!e.ctrlKey&&!e.metaKey){e.preventDefault(); if(typeof openMainSearchUI==='function') openMainSearchUI(); let si=document.getElementById('searchInput'); if(si) si.focus(); return;}
    if(e.key.toLowerCase()==='d'){e.preventDefault();toggleThemeV92();return;}
    if(e.key.toLowerCase()==='f'){e.preventDefault();toggleFocusModeV92();return;}
    if(e.key==='ArrowDown'){e.preventDefault();__v92PickGroup(1);return;}
    if(e.key==='ArrowUp'){e.preventDefault();__v92PickGroup(-1);return;}
    if(e.key==='+'||e.key==='='){e.preventDefault();setUiScaleV92(__v92GetScale()+5);return;}
    if(e.key==='-'){e.preventDefault();setUiScaleV92(__v92GetScale()-5);return;}
    if(e.altKey&&e.key==='1'){e.preventDefault();if(typeof openHome==='function')openHome();return;}
    if(e.altKey&&e.key==='2'){e.preventDefault();if(typeof openDatabase==='function')openDatabase('personal');return;}
    if(e.altKey&&e.key==='3'){e.preventDefault();if(typeof openDatabase==='function')openDatabase('auto');return;}
  });
})();

/* =========================================================
   V93 — Phase 10 Advanced Features (Scaffold)
   - Study Workspace: pin groups + open pinned list
   - Memorization Mode: progressive hide for shared text
   ========================================================= */
const V93_PINS_KEY='mutashabihat_v93_pinned_groups';
const V93_MEMO_KEY='mutashabihat_v93_memorize_level';

function getPinnedIdsV93(){
  try{
    let a=JSON.parse(localStorage.getItem(V93_PINS_KEY)||'[]');
    return Array.isArray(a)?a.map(Number).filter(n=>!isNaN(n)):[];
  }catch(e){return []}
}
function savePinnedIdsV93(ids){
  localStorage.setItem(V93_PINS_KEY,JSON.stringify((ids||[]).map(Number).filter(n=>!isNaN(n))));
}
function isPinnedV93(id){ return getPinnedIdsV93().includes(Number(id)); }
function togglePinGroupV93(id){
  id=Number(id);
  let ids=getPinnedIdsV93();
  if(ids.includes(id)) ids=ids.filter(x=>x!==id); else ids.push(id);
  savePinnedIdsV93(ids);
  if(typeof renderActiveGroups==='function') renderActiveGroups();
}

function openStudyWorkspaceV93(){
  let ids=getPinnedIdsV93();
  let data=(activeData&&activeData.length?activeData:personalData.concat(automatedData));
  let pinned=ids.map(id=>data.find(g=>Number(g.id)===Number(id))).filter(Boolean);
  let body=pinned.length
    ? `<div class="v93-study-grid">${pinned.map(g=>`<button class="v93-study-item" onclick="closeModal('studyWorkspaceV93');openDatabase('${activeDb||'personal'}');setTimeout(function(){var el=document.querySelector('#groups .group[data-id=&quot;${g.id}&quot;] .group-head');if(el)el.click();},180)"><b>${escapeHtml(g.title||('مجموعة '+g.id))}</b><small>#${escapeHtml(g.id)} — ${(g.verses||[]).length} آية</small></button>`).join('')}</div>`
    : `<div class="hint">لا توجد مجموعات مثبتة بعد. اضغط زر 📌 داخل أي بطاقة لإضافتها.</div>`;
  modal('studyWorkspaceV93','Study Workspace — المجموعات المثبتة',body,`<button onclick="closeModal('studyWorkspaceV93')">إغلاق</button>`);
}

function getMemoLevelV93(){
  let n=Number(localStorage.getItem(V93_MEMO_KEY)||0);
  if(isNaN(n)) n=0;
  return Math.max(0,Math.min(100,n));
}
function setMemoLevelV93(v){
  v=Math.max(0,Math.min(100,Number(v)||0));
  localStorage.setItem(V93_MEMO_KEY,String(v));
  applyMemorizationModeV93();
}
function shouldHideSharedTokenV93(index,level){
  if(level<=0) return false;
  if(level>=100) return true;
  let step=Math.max(1,Math.floor(100/Math.max(1,level)));
  return (index%step)===0;
}
function maskWordV93(word){
  let clean=safeText(word).trim();
  if(!clean) return clean;
  return '•'.repeat(Math.max(2,Math.min(8,clean.length)));
}
function applyMemorizationModeV93(){
  let level=getMemoLevelV93();
  document.body.classList.toggle('v93-memo-on',level>0);
  let parts=[...document.querySelectorAll('.verse-text .shared')];
  parts.forEach((el,i)=>{
    if(!el.dataset.v93Original) el.dataset.v93Original=el.textContent||'';
    if(level===0){ el.textContent=el.dataset.v93Original; return; }
    el.textContent=shouldHideSharedTokenV93(i,level)?maskWordV93(el.dataset.v93Original):el.dataset.v93Original;
  });
}
...
(function initV93(){
  ...
  if(typeof baseRenderCard==='function'){
    window.renderCard=function(g){
      let html=baseRenderCard.call(this,g);
      let pinCls=isPinnedV93(g.id)?' active':'';
      let pinBtn=`<button class="icon-btn outline-icon v93-pin-btn${pinCls}" title="تثبيت للدراسة" onclick="event.stopPropagation();togglePinGroupV93(${g.id})">📌</button>`;
      return html.replace('</div></div><div class="group-body">',pinBtn+'</div></div><div class="group-body">');
    };
  }
  const baseRenderActive=window.renderActiveGroups;
  if(typeof baseRenderActive==='function'){
    window.renderActiveGroups=function(){
      let r=baseRenderActive.apply(this,arguments);
      applyMemorizationModeV93();
      return r;
    };
  }
  const baseOpenSettings=window.openAppSettings;
  if(typeof baseOpenSettings==='function'){
    window.openAppSettings=function(){
      let r=baseOpenSettings.apply(this,arguments);
      setTimeout(function(){
        let body=document.querySelector('#settingsModal .modal-body');
        if(!body||body.querySelector('.v93-advanced-box')) return;
        let level=getMemoLevelV93();
        let box=document.createElement('div');
        box.className='settings-section v93-advanced-box';
        box.innerHTML=`<h2>Advanced Study Features (V93)</h2>
          <div class="inline-actions">
            <button onclick="openStudyWorkspaceV93()">فتح مساحة الدراسة</button>
            <button onclick="setMemoLevelV93(0)">إيقاف وضع الحفظ</button>
          </div>
          <label class="field">مستوى إخفاء النص المشترك (وضع الحفظ)
            <input type="range" min="0" max="100" value="${level}" oninput="setMemoLevelV93(this.value)">
          </label>`;
  ...
  window.addEventListener('DOMContentLoaded',function(){setTimeout(applyMemorizationModeV93,250)});
})();
