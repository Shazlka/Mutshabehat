...
function editNotesTabHtml(){return `<section class="v83-edit-section"><div class="v83-edit-section-head"><h3>الملاحظات</h3><small>نفس محرر الملاحظات الحالي</small></div><div class="v83-edit-notes-grid">${richEditor('editNote','ملاحظة','#1d4ed8')}${richEditor('editUnote','فائدة فريدة / إضافية','#b91c1c')}</div></section>`}

function renderEditTab(){let root=document.getElementById('editTabContent'),tabs=document.getElementById('editTabsMount');if(tabs)tabs.innerHTML=editTabsHtml();if(!root)return;if(editActiveTab==='ayahs'){root.innerHTML=editAyahsTabHtml();renderEditVerses();runQuranSearch('edit');return}if(editActiveTab==='notes'){root.innerHTML=editNotesTabHtml();let note=document.getElementById('editNote'),unote=document.getElementById('editUnote');if(note)note.innerHTML=editNoteBuffer;if(unote)unote.innerHTML=editUnoteBuffer;return}if(editActiveTab==='differences'){root.innerHTML=editPlaceholder('الفروقات');return}if(editActiveTab==='tags'){root.innerHTML=editPlaceholder('التصنيفات');return}root.innerHTML=editPlaceholder('المعاينة')}
