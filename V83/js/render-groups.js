...
function renderCard(g){
  ...
  if(editMode){
    actions+=`<button class="icon-btn outline-icon lock ${locked?'active':''}" title="قفل" onclick="event.stopPropagation();toggleFlag(${g.id},'locked')">${iconSvg('lock')}</button><button class="icon-btn outline-icon" title="مقارنة" onclick="event.stopPropagation();openCompareModal(${g.id})">${iconSvg('compare')}</button>${ro?`<button onclick="event.stopPropagation();copyAutoGroupToPersonal(${g.id})">نسخ للشخصية</button><button class="danger" title="حذف من القاعدة الآلية" onclick="event.stopPropagation();deleteAutoGroup(${g.id})">حذف من الآلية</button>`:`<button class="icon-btn outline-icon edit-action" title="تعديل" onclick="event.stopPropagation();openEditModal(${g.id})">${iconSvg('edit')}</button>`}`;
