...
## 2026-09-16
- **Feature — Canonical QIRAAT_COLORS Taxonomy & Hover Variant Popover Styling:** integrated the exact canonical color taxonomy for all 10 Imams and 20 Narrators (Ruwat) in `src/lib/qiraat-colors.ts` and connected it across the Mushaf viewer, floating hover popover, and detail modal.
  - *Taxonomy Architecture:* mapped each Imam to their canonical primary hue family with cohesive secondary shades for their two primary narrators (Nafi `#2563EB` -> Qalun `#60A5FA` / Warsh `#1D4ED8`; Ibn Kathir `#16A34A` -> al-Bazzi `#4ADE80` / Qunbul `#15803D`; Abu Amr `#0891B2` -> al-Duri `#67E8F9` / al-Susi `#0E7490`; Ibn Amir `#7C3AED` -> Hisham `#A78BFA` / Ibn Dhakwan `#6D28D9`; Asim `#EA580C` -> Shubah `#FB923C` / Hafs `#C2410C`; Hamzah `#DC2626` -> Khalaf `#F87171` / Khallad `#B91C1C`; al-Kisai `#DB2777` -> Abu al-Harith `#F472B6` / al-Duri `#BE185D`; Abu Ja'far `#CA8A04` -> Ibn Wardan `#FACC15` / Ibn Jammaz `#A16207`; Yaqub `#B45309` -> Ruways `#F59E0B` / Rawh `#92400E`; Khalaf al-Ashir `#475569` -> Ishaq `#94A3B8` / Idris `#334155`). Handled strict disambiguation for Khalaf (Hamzah rawi `#F87171` vs 10th Imam `#475569`) and al-Duri (under Abu Amr `#67E8F9` vs under al-Kisai `#BE185D`).
  - *Mushaf Viewer & Hover Popover:* marked disagreement words in Mushaf 1441 render with distinct crimson text color (`#8f1d14`), `font-bold`, warm gold background (`#fdf2d8`), and numbered badge `(١)`..`(٤)`. Hovering over any disagreement word displays `QiraatHoverTooltip` showing the word variants in large script alongside individual Qari badges styled with their exact taxonomy color token (`color`, `bg`, `border`, colored indicator dot).
  - *Detail Sheet:* updated `QiraatDetailSheet` attribution cards to display exact Imam and Rawi colors according to `QIRAAT_COLORS`.
  - *Tests & Screenshots:* expanded `tests/qiraat.test.mjs` (10/10 passed) to verify all 10 Imams and 20 Narrators and disambiguation resolution. Re-ran Playwright browser suite `tests/test_qiraat_browser.py` asserting hover popover with variants and Qari badges, capturing `docs/qiraat/screenshots/page-001/06-hover-word-variants-qaris.png`. Verified TypeScript (`npx tsc --noEmit` 0 errors), Next.js production build (29/29 routes), and `npm run mushaf:validate` (7/7 passed).
  - *Files Touched:* `src/lib/qiraat-colors.ts` (new), `src/components/qiraat/QiraatHoverTooltip.tsx`, `src/components/qiraat/QiraatDetailSheet.tsx`, `tests/qiraat.test.mjs`, `tests/test_qiraat_browser.py`, `docs/qiraat/screenshots/page-001/*`. No DB change.
- **Feature — Phase 1 Pilot: Qira'at Support for Mushaf 1441 (Page 1 / Surah Al-Fatihah ONLY):** added the pilot Qira'at (القراءات العشر الصغرى) annotation and viewer module strictly isolated to Page 1 / Surah Al-Fatihah from the authority source *"مصحف القراءات العشر بالرسم العثماني"* (Page 6 of PDF = Page 1 printed).
  - *Data Model & Database:* created normalized tables `qiraat_sources`, `qiraat_persons`, `qiraat_loci`, `qiraat_targets`, `qiraat_variants`, `qiraat_attributions` with RLS (`anon` and `authenticated` read-only) and supporting indexes. Seeded Page 1 data: 1 source, 16 persons (10 Imams, 4 Ruwat, and 2 composite authority groups), 4 loci, 5 targets, 10 variants, and 45 attributions. Applied migration to local database container `mutshabehat-db` (`supabase/migrations/20260916120000_qiraat_schema.sql`, `supabase/migrations/20260916121000_qiraat_page_001_seed.sql`).
  - *Disambiguation & Dual-Target Rules:* resolved Khalaf distinction: `KHALAF_ASHIR` (Imam #10) vs `KHALAF_HAMZA` (Rawi #1 of Hamza). Resolved Locus 4 (`عَلَيْهِمْ معًا`) via 1:N target mapping to both occurrences in 1:7 (`qurancom-word-8414` line 7 word 1 & `qurancom-word-8417` line 7 word 4).
  - *Backend Services & API:* added `src/types/qiraat.ts` and `src/lib/qiraat-service.ts` (`getQiraatByPage`, `getQiraatByWordId`, `getQiraatByAyahWord` with database query resilience and verified in-memory fallback). Added REST endpoints `GET /api/mushaf-1441/qiraat?page=1` (`src/app/api/mushaf-1441/qiraat/route.ts`) and `GET /api/mushaf-1441/qiraat/source-image?page=1` (`src/app/api/mushaf-1441/qiraat/source-image/route.ts`).
  - *Viewer Integration & UI/UX:* added toggle "القراءات" to `src/app/mushaf-1441/_components/Mushaf1441Viewer.tsx` (header button + drawer menu toggle), **OFF by default** ensuring zero layout shift or DOM diff on base text. When active, different words with Qira'at variants are styled with a distinct, dignified scholastic crimson text color (`#8f1d14`), `font-bold`, warm gold background (`#fdf2d8`), subtle border glow, and amber badges `(١)`..`(٤)`. Added interactive floating hover popover (`src/components/qiraat/QiraatHoverTooltip.tsx`) that appears instantly on hovering over any Qira'at word, displaying the different word variants in large Quranic script along with their respective Qaris (Imams in amber pills, Ruwat in emerald pills, and reading rules). Added `qiraatMode` prop to `MushafPageSlot` and updated custom memo comparison function. Tapping a badge or word opens responsive bottom-sheet/modal `src/components/qiraat/QiraatDetailSheet.tsx` displaying locus switcher pills, base word, variants, color-coded Imam (amber) vs Rawi (emerald) pills, Khalaf disambiguation, rule badges (e.g. كسر الهاء, إثبات الألف), verbatim source text, and scanned source page image preview.
  - *Tests & Verification:* added automated test suite `tests/qiraat.test.mjs` (9/9 passed: schema integrity, dual-target mapping, Khalaf separation, base text invariance, API endpoints, service lookups; added script `npm run test:qiraat`). Added browser Playwright integration test `tests/test_qiraat_browser.py` asserting distinct text color (`rgb(143, 29, 20)`), hover popover variants and Qaris, modal, dual-target, and mobile responsive behavior with 6 visual acceptance screenshots in `docs/qiraat/screenshots/page-001/` (including `06-hover-word-variants-qaris.png`). Generated data reconciliation report `docs/qiraat/page-001-reconciliation.md` (0 unresolved issues, 100% word match). Passed all 7 `npm run mushaf:validate` checks and `npx tsc --noEmit` (0 errors).
  - *Files Touched:* `src/types/qiraat.ts`, `src/lib/qiraat-service.ts`, `src/app/api/mushaf-1441/qiraat/route.ts`, `src/app/api/mushaf-1441/qiraat/source-image/route.ts`, `src/components/qiraat/QiraatDetailSheet.tsx`, `src/components/qiraat/QiraatHoverTooltip.tsx`, `src/app/mushaf-1441/page.tsx`, `src/app/mushaf-1441/_components/Mushaf1441Viewer.tsx`, `package.json`, `supabase/migrations/20260916120000_qiraat_schema.sql`, `supabase/migrations/20260916121000_qiraat_page_001_seed.sql`, `tests/qiraat.test.mjs`, `tests/test_qiraat_browser.py`, `docs/qiraat/page-001-reconciliation.md`. **DB migration:** `supabase/migrations/20260916120000_qiraat_schema.sql` and `supabase/migrations/20260916121000_qiraat_page_001_seed.sql`.
