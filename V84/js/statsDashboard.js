/**
 * statsDashboard.js — Full Statistics Dashboard Page (Tasks 3.4 + 4.3)
 *
 ...
 *   Task 4.2 — renderHeatmap()       (heatmap.js)
 * Plus a coloured detail grid mirroring the original dash-card data.
 *
 * Task 4.3 wiring: clicking a heatmap cell fires 'hm:surah-click' which
 * navigates to the workspace and applies filterBySurahNo() automatically.
 */
...
function _renderBody() {
  ...
  hmBox.className = 'sc-chart-box';

  const hmTitleRow = document.createElement('div');
  hmTitleRow.className = 'hm-title-row';

  const hmTitle = document.createElement('div');
  hmTitle.className = 'sc-chart-title';
  hmTitle.textContent = 'توزيع المجموعات على سور القرآن الكريم';

  const hmHint = document.createElement('div');
  hmHint.className = 'hm-click-hint';
  hmHint.textContent = 'انقر على خلية للتصفية';

  hmTitleRow.appendChild(hmTitle);
  hmTitleRow.appendChild(hmHint);
  hmBox.appendChild(hmTitleRow);
  renderHeatmap(hmBox);
  body.appendChild(hmBox);

  // 5 — Detail summary grid
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Task 4.3 — heatmap cell click: navigate to workspace + apply surah filter
  page.addEventListener('hm:surah-click', async e => {
    const { surahNumber, surahName } = e.detail;
    ...
    if (typeof window.openDatabase === 'function') {
      ...
      _setNavActive(db === 'personal' ? 'personal' : 'automated');
    }

    if (typeof window.filterBySurahNo === 'function') {
      await window.filterBySurahNo(surahNumber);
    }

    if (typeof window.toast === 'function') {
      window.toast(`المعروض: ${surahName}`, 'ok');
    }
  });
