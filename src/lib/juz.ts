// Juz boundaries — each juz starts at (surahNo, ayahNo).
// Source: standard Hafs ʿAdadi count. Pure data.
...
// Given a surah number, return the juz that contains its first ayah.
export function juzOfSurah(surahNo: number): number {
  for (let i = JUZ_STARTS.length - 1; i >= 0; i--) {
    if (JUZ_STARTS[i].surah <= surahNo) return JUZ_STARTS[i].juz
  }
  return 1
}
