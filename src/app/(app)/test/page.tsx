...
import { buildTestQuestions, TEST_MODES, type TestMode, type TestSourceGroup } from '@/lib/test-questions'

type SP = Promise<{ start?: string; scope?: string; surah?: string; count?: string; mode?: string }>
...
const MODE_OPTIONS: Record<TestMode, { label: string; hint: string }> = {
  mcq: { label: 'اختيار الموضع', hint: 'تظهر الآية فتختار سورتها ورقمها من بين مواضعها المتشابهة' },
  flash: { label: 'بطاقات الاستذكار', hint: 'يظهر الموضع فتستحضر الآية ثم تكشفها وتقيّم نفسك' },
  mixed: { label: 'مختلط', hint: 'مزيج من النوعين' },
}
...
export default async function TestPage({ searchParams }: { searchParams: SP }) {
  ...
  const settings = new URLSearchParams({ scope, mode, count: String(count), ...(surah ? { surah } : {}) })
  const settingsHref = `/test?${settings.toString()}`
  ...
  if (started) {
    ...
    const questions = buildTestQuestions((data as unknown as TestSourceGroup[]) ?? [], mode, count)
      ...
      <div className="max-w-2xl mx-auto px-3 md:px-8 py-5 md:py-12">
        <header className="mb-6 flex items-baseline justify-between gap-3">
          <div>
            ...
            <p className="mt-1.5 text-[12px] text-[var(--color-ink-muted)]">
              {MODE_OPTIONS[mode].label} · {SCOPES.find((s) => s.value === scope)?.label}{surah ? ` · سورة ${surah}` : ''}
            </p>
          </div>
        </header>
        ...
        {questions.length === 0 ? (
          <div className="py-16 px-6 text-center bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border-soft)]">
            ...
            <p className="mt-2 text-[12px] text-[var(--color-ink-muted)]">يحتاج الاختبار إلى مجموعات فيها موضعان مختلفان على الأقل.</p>
  ...
  const surahOptions = ((surahRows as { surah: string; cnt: number }[] | null) ?? [])
    .map((row) => ({ name: row.surah, count: Number(row.cnt), no: getSurahNumberByName(row.surah) ?? 999 }))
    .sort((a, b) => a.no - b.no)
    ...
    <div className="max-w-2xl mx-auto px-3 md:px-8 py-5 md:py-12">
      <header className="mb-8">
        ...
        <p className="mt-2 text-[13px] text-[var(--color-ink-muted)]">
          اختبر تمييزك بين المواضع المتشابهة من مجموعاتك الشخصية، ثم راجع الفروق الملوّنة بعد كل سؤال.
        </p>
      </header>

      <form action="/test" method="get" className="space-y-7">
        ...
        <fieldset>
          <legend className="text-[13px] font-bold text-[var(--color-ink)] mb-3">المجموعات</legend>
