...
function passFilters(g){let rawQ=(document.getElementById('searchInput')?.value||'').trim();let q=normalize(rawQ);let st=document.getElementById('statusFilter')?.value||'all';if(q){let gt=groupText(g);let matched=false;try{matched=smartArabicSearchMatch(rawQ,gt,50);}catch(e){matched=normalize(gt).includes(q);}if(!matched)return false;}if(!passStatus(g,st)||!passStatus(g,advancedFilters.status))return false;if(selectedSurahFilter&&!getTags(g).includes(selectedSurahFilter))return false;if(advancedFilters.kind&&safeText(g.candidateKind)!==advancedFilters.kind)return false;if(advancedFilters.minScore&&Number(g.candidateScore||0)<Number(advancedFilters.minScore))return false;if(advancedFilters.surah&&!getTags(g).some(s=>normalize(s).includes(normalize(advancedFilters.surah))))return false;return true}
