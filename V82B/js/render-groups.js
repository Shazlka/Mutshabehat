/* V90 — Dynamic serial numbering based on filtered results */
let __serialNumberMapV90 = new Map(); // Maps group ID to serial number

function buildSerialNumbersV90(list) {
  __serialNumberMapV90.clear();
  let counter = 1;
  list.forEach(g => {
    __serialNumberMapV90.set(g.id, counter);
    counter++;
  });
}

function getSerialNumberV90(groupId) {
  return __serialNumberMapV90.get(groupId) || groupId;
}
...
function renderGrouped(list){let m=new Map();list.forEach(g=>(getTags(g).length?getTags(g):['غير محدد']).forEach(s=>{if(!m.has(s))m.set(s,[]);m.get(s).push(g)}));let surahSections=[...m].sort((a,b)=>getSurahNo(a[0])-getSurahNo(b[0]));let globalCounter=1;return surahSections.map(([s,items])=>{let itemsHtml=items.map(g=>{let sn=globalCounter++;return renderCard(g,sn)}).join('');return `<section id="sec-${getSurahNo(s)}" class="surah-section"><div class="group-head" onclick="this.parentElement.classList.toggle('collapsed')"><div class="group-num">${getSurahNo(s)===9999?'؟':getSurahNo(s)}</div><div class="group-title-wrap"><div class="group-title">📖 سورة ${escapeHtml(s)}</div><div class="group-tags"><span class="tag">${items.length} مجموعة</span></div></div></div><div class="surah-section-groups">${itemsHtml}</div></section>`}).join('')}
...
function renderCard(g, serialNumber){
  let sn = serialNumber !== undefined ? serialNumber : getSerialNumberV90(g.id);
